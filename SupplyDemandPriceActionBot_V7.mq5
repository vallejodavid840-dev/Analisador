//+------------------------------------------------------------------+
//| SupplyDemandPriceActionBot_V7.mq5                                |
//| Educational EA: Supply & Demand + Price Action + Trade Management|
//| V7: entradas de continuación ancladas a la estructura real de     |
//|     swings (zigzag de máximos/mínimos), no a un simple toque de   |
//|     EMA, para evitar entradas en pleno rango lateral (chop) y     |
//|     encadenar operaciones legítimas mientras la tendencia siga.   |
//+------------------------------------------------------------------+
#property copyright "Botsito Optimizada"
#property version   "7.00"
#property strict
#property description "V7: la V6 entraba en continuación con solo tocar una EMA, lo que la metía en operaciones dentro de rangos laterales (chop) que terminaban en pérdida. Ahora la continuación de tendencia exige que la ESTRUCTURA de swings lo confirme: para comprar, el último mínimo (swing low) del TF de entrada tiene que ser más alto que el anterior (mínimos crecientes); para vender, el último máximo tiene que ser más bajo que el anterior (máximos decrecientes) -- igual que se traza a mano un zigzag marcando qué máximos/mínimos rompen la estructura y cuáles no. El SL se ancla a ese mismo swing (no a un rango arbitrario de N velas) y cada pivote de estructura solo se opera una vez, así que cuando una operación cierra (por el trailing de la V6) el bot puede abrir la siguiente en la próxima pierna de la tendencia sin repetir el mismo punto que ya usó."
...
input group "Continuación de tendencia (estructura de swings, señal extra)"
input bool InpUseTrendContinuation = true;    // suma entradas de continuación además de las zonas
input int InpEntrySwingDepth = 2;             // velas a cada lado (TF entrada) para confirmar un pivote
input int InpSwingLookbackBars = 80;          // hasta dónde busca los últimos 2 pivotes (TF entrada)
input bool InpRequireEmaPullback = false;     // exige además que el precio haya tocado la EMA de abajo
...
input int InpMinIndicatorConfirmations = 2;
...
input bool InpADXHardVeto = false;
...
input bool InpUseInfiniteTP = true;
input bool InpUseTrailingStop = true;
input double InpTrailStartAtrMultiplier = 0.8;
...
// NUEVO V7: recuerda el último pivote de estructura (swing) que ya se usó para abrir una
// operación de continuación, para no disparar varias veces sobre el mismo máximo/mínimo.
datetime last_buy_pivot_used = 0;
datetime last_sell_pivot_used = 0;
   ...
   if(entry_bar != last_entry_bar_time)
     {
      ...
      if(Bars(_Symbol, InpSignalTimeframe) >= InpLookbackBars + 20 && Bars(_Symbol, InpEntryTimeframe) >= InpSwingLookbackBars + 5)
         TryOpenTrade();
     }
...
// NUEVO V7: mismos fractales de swing que arriba, pero sobre el TF de entrada (M15) y con
// una profundidad propia (InpEntrySwingDepth), para dibujar el "zigzag" de máximos/mínimos
// que se usa como estructura de la continuación de tendencia.
bool IsEntrySwingHigh(const int shift)
  {
   double value = iHigh(_Symbol, InpEntryTimeframe, shift);
   for(int i = 1; i <= InpEntrySwingDepth; i++)
      if(iHigh(_Symbol, InpEntryTimeframe, shift - i) >= value || iHigh(_Symbol, InpEntryTimeframe, shift + i) >= value)
         return false;
   return true;
  }

bool IsEntrySwingLow(const int shift)
  {
   double value = iLow(_Symbol, InpEntryTimeframe, shift);
   for(int i = 1; i <= InpEntrySwingDepth; i++)
      if(iLow(_Symbol, InpEntryTimeframe, shift - i) <= value || iLow(_Symbol, InpEntryTimeframe, shift + i) <= value)
         return false;
   return true;
  }

// Devuelve los últimos dos pivotes (máximos o mínimos) confirmados del TF de entrada, con
// su hora, para poder comparar "¿el último es más alto/bajo que el anterior?" (estructura
// de tendencia) y anclar el SL justo detrás del pivote más reciente.
bool GetLastTwoSwingHighs(double &last_val, datetime &last_time, double &prev_val, datetime &prev_time)
  {
   int found = 0;
   last_val = 0; prev_val = 0; last_time = 0; prev_time = 0;
   for(int shift = InpEntrySwingDepth + 1; shift < InpSwingLookbackBars; shift++)
     {
      if(IsEntrySwingHigh(shift))
        {
         if(found == 0) { last_val = iHigh(_Symbol, InpEntryTimeframe, shift); last_time = iTime(_Symbol, InpEntryTimeframe, shift); found++; }
         else { prev_val = iHigh(_Symbol, InpEntryTimeframe, shift); prev_time = iTime(_Symbol, InpEntryTimeframe, shift); found++; break; }
        }
     }
   return found >= 2;
  }

bool GetLastTwoSwingLows(double &last_val, datetime &last_time, double &prev_val, datetime &prev_time)
  {
   int found = 0;
   last_val = 0; prev_val = 0; last_time = 0; prev_time = 0;
   for(int shift = InpEntrySwingDepth + 1; shift < InpSwingLookbackBars; shift++)
     {
      if(IsEntrySwingLow(shift))
        {
         if(found == 0) { last_val = iLow(_Symbol, InpEntryTimeframe, shift); last_time = iTime(_Symbol, InpEntryTimeframe, shift); found++; }
         else { prev_val = iLow(_Symbol, InpEntryTimeframe, shift); prev_time = iTime(_Symbol, InpEntryTimeframe, shift); found++; break; }
        }
     }
   return found >= 2;
  }
...
// NUEVO V7: la continuación de tendencia ya no dispara con solo tocar una EMA (eso metía al
// bot en operaciones dentro de rangos laterales, como la compra perdedora de la captura).
// Ahora exige que la ESTRUCTURA de swings del TF de entrada confirme la tendencia:
//  - Para comprar: el último mínimo confirmado debe ser MÁS ALTO que el mínimo anterior
//    (mínimos crecientes = la tendencia alcista sigue intacta, como el "check" en el dibujo).
//  - Para vender: el último máximo confirmado debe ser MÁS BAJO que el máximo anterior
//    (máximos decrecientes, como las "X" marcadas en los techos que se rompieron).
// El SL se ancla justo detrás de ese pivote (no a un rango arbitrario de N velas) y cada
// pivote solo dispara una operación una vez (last_buy_pivot_used/last_sell_pivot_used), así
// que cuando el trailing de la V6 cierra la operación en curso, el bot puede abrir la
// siguiente en la próxima pierna de la tendencia sin repetir el mismo punto.
void TryTrendContinuationTrade()
  {
   ...
   if(InpUseHTFTrendFilter)
     {
      int htrend = GetHigherTFTrend();
      if(htrend != 0 && ((want_sell && htrend > 0) || (!want_sell && htrend < 0))) return;
     }
   ...
   if(want_sell)
     {
      double last_high, prev_high;
      datetime last_time, prev_time;
      if(!GetLastTwoSwingHighs(last_high, last_time, prev_high, prev_time)) return;
      if(last_high >= prev_high) return;              // necesito un máximo más bajo (estructura bajista intacta)
      if(last_time == last_sell_pivot_used) return;    // este pivote ya se operó, esperar el siguiente
      sl_raw = last_high;
      pivot_time = last_time;
     }
   else
     {
      double last_low, prev_low;
      datetime last_time, prev_time;
      if(!GetLastTwoSwingLows(last_low, last_time, prev_low, prev_time)) return;
      if(last_low <= prev_low) return;                 // necesito un mínimo más alto (estructura alcista intacta)
      if(last_time == last_buy_pivot_used) return;
      sl_raw = last_low;
      pivot_time = last_time;
     }
   ...
   if(want_sell && close1 > open1) return; // la vela de gatillo debe cerrar a favor de la venta
   if(!want_sell && close1 < open1) return; // y a favor de la compra

   if(InpRequireEmaPullback)
     {
      double ema[];
      ...
      if(!(high1 >= ema[0] && low1 <= ema[0])) return;
     }
   ...
   if(SendTrade(want_sell, sl_raw, want_sell ? "Trend structure sell" : "Trend structure buy"))
     {
      if(want_sell) last_sell_pivot_used = pivot_time;
      else last_buy_pivot_used = pivot_time;
     }
  }
...
void UpdateDashboard()
  {
   ...
   string txt = "";
   txt += "=== SupplyDemandPriceActionBot V7 (estructura de swings) ===\n";
   ...
   bool has_highs = GetLastTwoSwingHighs(lh, lht, ph, pht);
   bool has_lows = GetLastTwoSwingLows(ll, llt, pl, plt);
   txt += "--- Estructura (" + TimeframeToString(InpEntryTimeframe) + ") ---\n";
   if(has_highs)
      txt += StringFormat("Máximos: %s -> %s  %s\n", DoubleToString(ph, _Digits), DoubleToString(lh, _Digits),
                           lh < ph ? "(decrecientes, bajista)" : "(crecientes)");
   if(has_lows)
      txt += StringFormat("Mínimos: %s -> %s  %s\n", DoubleToString(pl, _Digits), DoubleToString(ll, _Digits),
                           ll > pl ? "(crecientes, alcista)" : "(decrecientes)");
